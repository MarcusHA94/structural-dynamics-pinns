...
class beam_sep_ww_pinn(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.n_input = 1
        self.n_output_disp = self.n_modes
        self.n_hidden_disp = config["n_hidden"]
        self.n_layers_disp = config["n_layers"]
    ...
    def build_disp_net(self):
        self.net_state = nn.Sequential(
            nn.Sequential(*[nn.Linear(self.n_input, self.n_hidden_disp), self.activation()]),
            nn.Sequential(*[nn.Sequential(*[nn.Linear(self.n_hidden_disp, self.n_hidden_disp), self.activation()]) for _ in range(self.n_layers_disp - 1)]),
            nn.Linear(self.n_hidden_disp, self.n_output_disp)
            )
    ...
    def loss_func(self, lambdas):
        ...
        if self.switches['obs']:
            ...
            q_obs_pred, phi_obs_pred = self.forward(self.t_obs, self.x_obs)
            w_obs_pred = torch.sum(torch.matmul(q_obs_pred.unsqueeze(2), phi_obs_pred.T.unsqueeze(1)), dim=0).squeeze()
            R_obs = torch.sum(torch.sqrt(torch.sum((w_obs_pred - self.w_obs)**2, dim=2)), dim=0)
        
        if self.switches['ode'] or self.switches['cc'] or self.switches['bc']:
            # generate prediction over collocation domain
            q_col_pred, phi_col_pred = self.forward(self.t_col, self.x_col)
            q_col_pred = q_col_pred.squeeze()

            # retrieve derivatives
            dq_dt = torch.zeros_like(q_col_pred)
            d2q_dt2 = torch.zeros_like(q_col_pred)
            for i in range(q_col_pred.shape[1]):
                dq_dt[:,i] = torch.autograd.grad(q_col_pred[:,i], self.t_col, torch.ones_like(q_col_pred[:,i]), create_graph=True)[0][:,0]  # ∂_t-hat N_q-hat
                d2q_dt2[:,i] = torch.autograd.grad(dq_dt[:,i], self.t_col, torch.ones_like(dq_dt[:,i]), create_graph=True)[0][:,0]  # ∂_t-hat^2 N_q-hat

            d4phi_dx4 = torch.zeros_like(phi_col_pred)
            for i in range(phi_col_pred.shape[1]):
                dphi_dx = torch.autograd.grad(phi_col_pred[:,i], self.x_col, torch.ones_like(phi_col_pred[:,i]), create_graph=True)[0][:,0]  # ∂_x-hat N_φ-hat
                d2phi_dx2 = torch.autograd.grad(dphi_dx, self.x_col, torch.ones_like(dphi_dx), create_graph=True)[0][:,0]  # ∂_x-hat^2 N_φ-hat
                d3phi_dx3 = torch.autograd.grad(d2phi_dx2, self.x_col, torch.ones_like(d2phi_dx2), create_graph=True)[0][:,0]  # ∂_x-hat^3 N_φ-hat
                d4phi_dx4[:,i] = torch.autograd.grad(d3phi_dx3, self.x_col, torch.ones_like(d3phi_dx3), create_graph=True)[0][:,0]  # ∂_x-hat^4 N_φ-hat
